...
export class DnsComponent {
  ...
  upstreamType = signal<'isp' | 'cloudflare' | 'google' | 'quad9' | 'custom'>('isp');
  primaryDns = signal('');
  secondaryDns = signal('');
    ...
    effect(() => {
      ...
      if (cfg) {
        untracked(() => {
          ...
          } else if (cfg.upstream[0] === '1.1.1.1' && cfg.upstream[1] === '1.0.0.1') {
            this.upstreamType.set('cloudflare');
          } else if (cfg.upstream[0] === '8.8.8.8' && cfg.upstream[1] === '8.8.4.4') {
            this.upstreamType.set('google');
          } else if (cfg.upstream[0] === '9.9.9.9' && cfg.upstream[1] === '149.112.112.112') {
  ...
  saveConfig() {
    ...
    switch (this.upstreamType()) {
      case 'isp': upstream = []; break;
      case 'cloudflare': upstream = ['1.1.1.1', '1.0.0.1']; break;
      case 'google': upstream = ['8.8.8.8', '8.8.4.4']; break;
      case 'quad9': upstream = ['9.9.9.9', '149.112.112.112']; break;
      case 'custom':
    ...
    const cfg: DnsConfig = {
      upstream,
      local_domain: this.localDomainEnabled() ? (this.localDomain() || 'home.local') : null,
